<div align="center">
  ...
  <p>
    <a href="https://github.com/moronfranklyn-lab/panel-xiaobei/releases/latest"><strong>下载 macOS 版</strong></a>
    ·
    <a href="https://github.com/moronfranklyn-lab/panel-xiaobei/releases/latest"><strong>下载 Windows 版</strong></a>
    ·
    <a href="#从源码运行">从源码运行</a>
    ·
    <a href="#更新日志">更新日志</a>
    ·
    <a href="https://github.com/moronfranklyn-lab/panel-xiaobei/issues">反馈问题</a>
  </p>
  <p>
    <img alt="Release" src="https://img.shields.io/github/v/release/moronfranklyn-lab/panel-xiaobei?style=flat-square&color=7c8cff" />
...
| 页面 | 解决什么问题 |
| --- | --- |
| **首页** | 当前窗口、镜子、快速录音、随笔记、常用指令、本地音乐控制和番茄钟集中在一个 Bento 工作台 |
...
| **宠物** | 查看桌宠的完整对话历史与长期记忆，可手动新增、删除或清空记忆条目 |
| **设置** | 当菜单栏图标被刘海遮挡时，仍可在面板内配置 API、外观主题、桌宠、镜子、首页组件、功能显示、默认展开页、快捷键、数据目录与开机启动 |

剪贴板历史默认关闭，可从菜单栏或面板「设置」的「显示功能」中按需启用。菜单栏入口与设置页读写同一份本机配置；即使状态栏图标过多、被物理刘海遮挡，也不影响调整。Codex、Claude Code 与 GPT 的本机完成事件也可以直接显示为不抢焦点的顶部提醒。首页的本地音乐卡片会自动选择已安装的客户端（优先网易云音乐），支持播放/暂停与切歌。

「设置 → 外观」可以在经典黑与照片主题之间切换：上传一张照片，应用会用原生 canvas 提取主色板生成整套配色，并按 WCAG 公式实算正文对比度、自动校正到 4.5:1 以上，因此不用担心浅色照片让文字看不清。圆角、间距、字体与动效取值完全不受换肤影响，经典黑也可随时一键恢复；主题会同步应用到展开面板、任务提醒与桌宠，重启后保持且无首帧闪烁。

桌宠默认开启，使用小贝（布偶猫形象，素材位于 `renderer/assets/pet/`）在桌面上自由走动。动作按场景自动切换：静息用 Idle，走路使用同一侧身母版生成的 8 帧四足 Walk，并按真实移动距离同步脚步、平滑加速和刹停；走完后保持 6–14 秒静息，Sleepy 只偶尔出现。**单击会先播放被摸头（Petting）再弹出对话气泡**，双击播放引导手势（Guiding）并展开「宠物」页；任务完成播 Success，番茄钟与对话回复播 Satisfied，待办提醒与未配置时一脸疑惑（Uncertain），等待回答时思考（Thinking），气泡开着等输入时 Waiting。对话复用本机 DeepSeek 配置与加密密钥，不需要再填一次 API Key；对话历史与长期记忆保存在工作区的 `pet-memory.json`。可在「设置 → 桌宠」关闭桌宠或在 64 / 96 / 128 px 三档中调整尺寸（默认 96）；宠物是一个只有宠物大小的小窗口（约 282×120，随尺寸档位变化）并常驻最前：能看到、能点击、能按住拖到桌面任意位置，松手后位置被记住；窗口空白处鼠标穿透，不影响桌面操作。气泡展开时窗口才向上加高，宠物本身不会移动。桌宠不会覆盖全屏应用，可随 Space 切换跟随；按住宠物即可拖到桌面任意位置，松手后位置被记住，下一次启动仍在原处。
...
| --- | --- |
| Mac | [TO-DO-Panel-1.1.2-arm64.dmg](https://github.com/moronfranklyn-lab/panel-xiaobei/releases/download/v1.1.2/TO-DO-Panel-1.1.2-arm64.dmg) |
| Windows | [TO-DO-Panel-1.1.2-windows-x64-setup.exe](https://github.com/moronfranklyn-lab/panel-xiaobei/releases/download/v1.1.2/TO-DO-Panel-1.1.2-windows-x64-setup.exe) |
...
Windows 首版暂不显示「当前窗口」和「本地音乐」组件；剪贴板点击复制后用 Ctrl+V 粘贴，AI 完成提醒暂不支持点击切回任务窗口。平台限制不覆盖原有组件偏好。镜子仅主动点击后开启，离开首页或收起立即释放；结束录音释放麦克风。系统拒绝访问设备时，请检查 Windows 隐私设置中桌面应用的相机 / 麦克风权限。
...
```bash
git clone https://github.com/moronfranklyn-lab/panel-xiaobei.git
cd TO-DO-Panel
npm install
npm test
npm start
...
[MIT](LICENSE) © 2026 [moronfranklyn-lab](https://github.com/moronfranklyn-lab)
