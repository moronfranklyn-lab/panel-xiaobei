...
contextBridge.exposeInMainWorld('notchAPI', {
  ...
  requestScreenRecording: () => ipcRenderer.invoke('privacy:request-screen-recording'),
  ...
  // 照片生成换肤：色板提取在渲染层完成，主进程只负责选图落盘与校验持久化。
  setTheme: (payload) => ipcRenderer.invoke('settings:set-theme', payload),
  choosePhotoTheme: () => ipcRenderer.invoke('theme:choose-photo'),
  getPhotoTheme: () => ipcRenderer.invoke('theme:get-photo'),
  clearPhotoTheme: () => ipcRenderer.invoke('theme:clear-photo'),
  onThemeChanged: (cb) => subscribe('theme:changed', (event, payload) => cb(payload)),
  // 桌宠
  setPetSettings: (payload) => ipcRenderer.invoke('settings:set-pet', payload),
  petState: () => ipcRenderer.invoke('pet:state'),
  petReportLayout: (payload) => ipcRenderer.invoke('pet:report-layout', payload),
  petMove: (payload) => ipcRenderer.invoke('pet:move', payload),
  petSetChrome: (payload) => ipcRenderer.invoke('pet:set-chrome', payload),
  petChat: (payload) => ipcRenderer.invoke('pet:chat', payload),
  petMemoryGet: () => ipcRenderer.invoke('pet:memory-get'),
  petMemoryAdd: (payload) => ipcRenderer.invoke('pet:memory-add', payload),
  ...
  petSetInteractive: (payload) => ipcRenderer.invoke('pet:set-interactive', payload),
  petSetFocusable: (payload) => ipcRenderer.invoke('pet:set-focusable', payload),
  petOpenPanel: () => ipcRenderer.invoke('pet:open-panel'),
  petSaveMotion: (payload) => ipcRenderer.invoke('pet:save-motion', payload),
  onPetConfig: (cb) => subscribe('pet:config', (event, payload) => cb(payload)),
  onPetTheme: (cb) => subscribe('pet:theme', (event, payload) => cb(payload)),
  onPetEvent: (cb) => subscribe('pet:event', (event, payload) => cb(payload)),
  onPetMemoryChanged: (cb) => subscribe('pet:memory-changed', (event, payload) => cb(payload)),
  onOpenPet: (cb) => subscribe('app:open-pet', () => cb()),
