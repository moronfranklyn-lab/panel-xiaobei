(function initWorkspace() {
  ...
  function renderWindows(error = '') {
    ...
    if (error) {
      ...
      hint.textContent = screenRecording
        ? '点「请求授权」会弹出系统授权框；macOS 只有在应用真正发起过一次捕获后才会把它列进「屏幕录制」，授权后需重启应用。'
        : `系统设置 → 隐私与安全性 → ${pane}，允许 TO-DO Panel 后重试。`;
      const action = document.createElement('button');
      action.type = 'button';
      action.className = 'window-permission-open';
      if (screenRecording) {
        // 直接跳系统设置是死胡同（列表里还没有条目），必须先触发一次真实捕获
        // 让 macOS 完成登记，再引导用户去打开开关。
        action.textContent = '请求授权';
        action.addEventListener('click', async () => {
          action.disabled = true;
          try {
            if (window.notchAPI && typeof window.notchAPI.requestScreenRecording === 'function') {
              await window.notchAPI.requestScreenRecording();
            }
          } catch (error) {
            // 授权框被拒绝或取消都不算异常，继续走下面的设置页引导。
          } finally {
            action.disabled = false;
          }
          if (window.notchAPI && typeof window.notchAPI.openPrivacySettings === 'function') {
            window.notchAPI.openPrivacySettings('screen-recording');
          }
        });
      } else {
  ...
  async function refreshMusicStatus() {
    ...
    // 客户端名称由主进程按安装情况下发（label），渲染层不硬编码品牌名
    const musicLabel = status && status.label ? status.label : '本地音乐';
    if (musicTitle) musicTitle.textContent = status && status.installed ? musicLabel : `未安装${musicLabel}`;
  ...
  homeMusic?.addEventListener('click', async (event) => {
    ...
    if (!result || !result.ok) {
      const needsSession = result && ['no_active_session', 'music_session_inactive'].includes(result.error);
      ...
      if (typeof showStatusToast === 'function') {
        showStatusToast(result && result.error === 'not_installed'
          ...
          : needsPermission ? '请在系统设置中允许 TO-DO Panel 使用辅助功能'
            : needsSession ? '请先点击播放，再使用切歌控制' : `${musicLabel}控制暂不可用`);
      }
    } else {
