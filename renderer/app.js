...
// 点击桌面上的桌宠（双击或气泡里的入口）→ 展开面板并切到「宠物」页。
if (window.notchAPI && typeof window.notchAPI.onOpenPet === 'function') {
  window.notchAPI.onOpenPet(async () => {
    await setActiveTab('pet');
    if (!isExpanded) await setMode(true);
  });
}
...
const ALL_TABS = ['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'clip', 'pet', 'settings'];
...
function applyFeatureSettings(settings) {
  // 桌宠不是「功能开关」而是独立偏好（settings.pet.enabled），
  // 但 Tab 可见性走同一套判定，所以在这里折算进 features。
  const features = {
    ...(settings && settings.features || {}),
    home: true,
    settings: true,
    pet: !(settings && settings.pet && settings.pet.enabled === false),
  };
