...
subscribe('onThemeChanged', applyTheme);

// 提醒窗口不落盘、不读配置：主题完全由主进程推过来，推不到就维持经典黑兜底。
function applyTheme(payload) {
  if (!window.NotchTheme) return;
  const theme = payload && payload.theme === 'photo' ? 'photo' : 'classic';
  const palette = payload && payload.palette;
  if (theme === 'photo' && palette) window.NotchTheme.apply(palette, 'notification');
  else window.NotchTheme.clear();
}
