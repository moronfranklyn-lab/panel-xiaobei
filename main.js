...
const {
  ...
  normalizePetMotion,
  truncatePetMemory,
  appendPetConversation,
  buildPetChatMessages,
...
const TAB_SIZES = {
  ...
  pet: { width: EXPANDED_WIDTH, panelHeight: EXPANDED_PANEL_HEIGHT },
...
const PHOTO_THEME_FILE = 'photo-theme.jpg';
const PET_MEMORY_FILE = 'pet-memory.json';
const workspacePersistenceGate = createWorkspacePersistenceGate();
// 音乐控制目标：按安装情况择一。界面名称由主进程下发，换客户端不必改渲染层。
const MUSIC_APPS = [
  { path: '/Applications/NeteaseMusic.app', bundleId: 'com.netease.163music', label: '网易云音乐' },
  { path: '/Applications/汽水音乐.app', bundleId: 'com.soda.music', label: '汽水音乐' },
];

function resolveMusicApp() {
  return MUSIC_APPS.find((candidate) => fs.existsSync(candidate.path)) || null;
}
...
// 桌宠：整屏透明覆盖窗口，窗口边界创建后不再变化，宠物走动全部由渲染层完成。
let petWindow = null;
let petWindowReady = false;
let petMemoryCache = null;
...
// 屏幕录制的系统授权框每次启动最多触发一次，避免轮询「当前窗口」时反复弹窗。
let screenRecordingPromptRequested = false;
...
function enqueueTaskNotification(notification) {
  ...
  // 桌宠事件联动的唯一入口：去重通过后才让宠物欢呼，重复提醒不会重复触发动作。
  const petEventKinds = { todo: 'todo', pomodoro: 'pomodoro' };
  sendPetEvent({
    kind: petEventKinds[notification.source] || 'task',
    title: String(notification.title || '任务已完成'),
    body: String(notification.body || notification.detail || notification.project || ''),
    source: String(notification.source || 'task'),
  });
...
function createTaskNotificationWindow() {
  ...
  targetWindow.webContents.once('did-finish-load', () => {
    ...
    // 提醒窗口是独立 BrowserWindow，页面加载完先把当前主题推过去，
    // 否则换肤后弹出的提醒会停留在经典黑。
    const settings = readAppSettings();
    targetWindow.webContents.send('theme:changed', {
...
// ============ 桌宠 ============
// 整屏透明覆盖窗口：边界创建后不再变化，宠物走动全部是窗口内的 CSS transform，
// 因此不会触碰「窗口边界零动画」这条防卡顿铁律。默认鼠标穿透，
// 只有渲染层命中宠物身体时才临时恢复点击。
// 桌宠窗口只有宠物那么大（外加气泡/提示卡按需扩展的空间），不再做全屏覆盖层。
// 这样它既能常驻最前被点击拖动，又只占一小块地方，不会盖住用户的内容。
const PET_BASE_PAD = 12;
let petLayout = { petSize: 96, bubbleWidth: 258, toastWidth: 236 };
let petBaseSize = { width: 140, height: 120 };
let petChrome = { width: 0, height: 0 };

function petWorkArea(display) {
  const d = display || getTargetDisplay();
  const area = d && d.workArea ? d.workArea : d && d.bounds;
  return area
    ? { x: Math.round(area.x), y: Math.round(area.y), width: Math.round(area.width), height: Math.round(area.height) }
    : { x: 0, y: 0, width: 1440, height: 900 };
}

function computePetBaseSize() {
  const pet = Math.max(32, Number(petLayout.petSize) || 96);
  const widest = Math.max(pet, Number(petLayout.bubbleWidth) || 258, Number(petLayout.toastWidth) || 236);
  return {
    petSize: Math.round(pet),
    width: Math.round(Math.max(pet + PET_BASE_PAD * 2, widest + PET_BASE_PAD * 2)),
    height: Math.round(pet + PET_BASE_PAD * 2),
  };
}
...
function currentPetWindowSize() {
  ...
  return {
    width: Math.min(area.width, Math.round(petBaseSize.width + petChrome.width)),
    height: Math.min(area.height, Math.round(petBaseSize.height + petChrome.height)),
  };
}

// 保存的是 0..1 归一化位置，按「工作区内可移动范围」换算成窗口左上角屏幕坐标。
function petBoundsFromMotion(motion, size) {
  const area = petWorkArea();
  const width = Math.min(area.width, size.width);
  const height = Math.min(area.height, size.height);
  const spanX = Math.max(0, area.width - width);
  const spanY = Math.max(0, area.height - height);
  const normalized = normalizePetMotion(motion);
  return {
    x: Math.round(area.x + normalized.x * spanX),
    y: Math.round(area.y + normalized.y * spanY),
    width,
    height,
  };
}
...
function getPetBounds(display) {
  ...
  return petBoundsFromMotion(readAppSettings().petMotion, size);
}

// 窗口尺寸变化时保持「宠物的屏幕位置」不变：宠物锚在窗口底部居中，
// 所以向上加高要同步下移 y，向两侧加宽要同步左移 x。
function applyPetWindowSize(next) {
  if (!petWindow || petWindow.isDestroyed()) return null;
  const area = petWorkArea();
  const current = petWindow.getBounds();
  const width = Math.min(area.width, Math.max(petBaseSize.width, Math.round(next.width)));
  const height = Math.min(area.height, Math.max(petBaseSize.height, Math.round(next.height)));
...
function petPublicConfig() {
  const settings = readAppSettings();
  const llmConfig = resolveLlmConfig();
  const area = petWorkArea();
  const size = currentPetWindowSize();
  const bounds = petWindow && !petWindow.isDestroyed() ? petWindow.getBounds() : getPetBounds();
  return {
    ...
    windowSize: size,
    bounds,
    petSize: petBaseSize.petSize,
  };
}

function sendPetConfig() {
  if (!petWindowReady || !petWindow || petWindow.isDestroyed()) return;
  petWindow.webContents.send('pet:config', petPublicConfig());
}

function sendPetEvent(payload) {
  if (!petWindowReady || !petWindow || petWindow.isDestroyed()) return;
  petWindow.webContents.send('pet:event', payload);
}

function createPetWindow() {
  if (petWindow && !petWindow.isDestroyed()) return petWindow;
  petWindowReady = false;
  petBaseSize = computePetBaseSize();
  ...
  petWindow = new BrowserWindow({
    ...
    resizable: false,
    movable: false,
    // 默认不可聚焦：覆盖整屏的窗口一旦抢焦点，用户当前的工作就断了。
    // 只有气泡输入打开时，渲染层才会申请临时聚焦。
    focusable: false,
    // 常驻最前，但窗口只有宠物那么大：既能随时看见、点击、拖动，
    // 又不会像全屏覆盖层那样盖住用户的内容。
    // （macOS 真正的桌面背景层 type:'desktop' 不接收鼠标事件，宠物会完全点不到；
    //   全屏覆盖层放在普通层级又会被窗口整片盖住。小窗口是唯一两者兼顾的形态。）
    alwaysOnTop: true,
    skipTaskbar: true,
    hasShadow: false,
    hiddenInMissionControl: true,
  ...
  // floating 层级：浮在普通窗口之上，但低于主面板（screen-saver level 1）。
  petWindow.setAlwaysOnTop(true, 'floating');
  // 跟随 Space 切换，但不覆盖全屏应用（全屏看视频时不该有只猫在上面）。
  if (process.platform === 'darwin') petWindow.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: false });
  petWindow.setIgnoreMouseEvents(true, { forward: true });
  petWindow.loadFile(path.join(__dirname, 'renderer', 'pet.html'));

  targetWindow.webContents.once('did-finish-load', () => {
    if (petWindow !== targetWindow || targetWindow.isDestroyed()) return;
    petWindowReady = true;
  ...
  // 右键桌宠给一个就近出口：隐藏桌宠 / 打开面板 / 退出应用
  targetWindow.webContents.on('context-menu', () => {
    if (petWindow !== targetWindow || targetWindow.isDestroyed()) return;
    Menu.buildFromTemplate([
      {
        ...
        click: () => {
          ...
          saveAppSettings({ ...current, pet: normalizePetSettings({ ...current.pet, enabled: false }) });
          syncPetWindow();
          if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('settings:changed', publicAppSettings());
        },
      },
      { label: '打开宠物面板', click: () => openRendererPanel('app:open-pet') },
      { type: 'separator' },
      { label: '退出 TO-DO Panel', click: () => app.quit() },
    ]).popup({ window: targetWindow });
  });
  targetWindow.on('closed', () => {
    if (petWindow !== targetWindow) return;
    petWindow = null;
    petWindowReady = false;
  });
  return petWindow;
...
// 桌宠开关与尺寸的唯一收敛点：关闭即销毁窗口，开启即创建或下发新配置。
function syncPetWindow() {
  ...
  // 尺寸档位变化时窗口要跟着变，否则宠物会被裁切
  applyPetWindowSize({ width: petBaseSize.width + petChrome.width, height: petBaseSize.height + petChrome.height });
  sendPetConfig();
}
...
function petMemoryPath() {
  return workspacePath(PET_MEMORY_FILE);
}

function readPetMemory() {
  if (petMemoryCache) return petMemoryCache;
  petMemoryCache = truncatePetMemory(readJsonFile(petMemoryPath(), { conversations: [], longTerm: [] }));
  return petMemoryCache;
}

function writePetMemory(memory) {
  const next = truncatePetMemory(memory);
  if (!writeJsonFile(petMemoryPath(), next)) return null;
  petMemoryCache = next;
  return next;
}

function broadcastPetMemory() {
  const payload = readPetMemory();
  for (const target of [mainWindow, petWindow]) {
    if (target && !target.isDestroyed()) target.webContents.send('pet:memory-changed', payload);
  }
}

async function requestPetReply(message) {
  const question = String(message == null ? '' : message).trim().slice(0, 2000);
  if (!question) return { ok: false, error: 'empty_message' };
  ...
  try {
    const response = await fetch(safeEndpoint, {
      ...
      body: JSON.stringify({
        ...
        messages: buildPetChatMessages(memory, question),
      }),
    });
    ...
    if (!reply) return { ok: false, error: 'invalid_response' };
    const now = Date.now();
    const withQuestion = appendPetConversation(memory, 'user', question, { id: `pet-${now}-u`, ts: now });
    const withReply = appendPetConversation(withQuestion, 'assistant', reply, { id: `pet-${now}-a`, ts: now + 1 });
    writePetMemory(withReply);
    broadcastPetMemory();
...
// 主题变更要同时落到主面板、提醒窗口与桌宠，任何一处漏掉都会出现「半套颜色」。
function broadcastTheme() {
  const settings = readAppSettings();
  const payload = { theme: settings.theme, palette: settings.themePalette };
  for (const target of [mainWindow, notificationWindow]) {
    if (target && !target.isDestroyed()) target.webContents.send('theme:changed', payload);
  }
  if (petWindowReady && petWindow && !petWindow.isDestroyed()) {
    petWindow.webContents.send('pet:theme', payload);
  }
}
...
function readAppSettings() {
  ...
  const theme = normalizeThemeSettings(stored.theme, stored.themePalette);
  const pet = normalizePetSettings(stored.pet);
  // 桌宠开关不是 feature，但「默认展开页」的有效性判定要把它算进去，
  // 否则关闭桌宠后仍会把「宠物」当成合法默认页。
  const tabsForDefault = { ...features, pet: pet.enabled };
  return {
    features,
    shortcut: isValidPanelShortcut(stored.shortcut) ? stored.shortcut : 'Space',
    defaultTab: normalizeDefaultTabPreference(stored.defaultTab, tabsForDefault),
    theme: theme.theme,
    themePalette: theme.palette,
    pet,
    petMotion: normalizePetMotion(stored.petMotion),
  };
}
...
function copyWorkspaceAssets(sourceRoot, targetRoot) {
  ...
  for (const filename of [WORKSPACE_DATA_FILE, MIRROR_IMAGE_FILE, PHOTO_THEME_FILE, PET_MEMORY_FILE]) {
...
async function chooseWorkspaceFolder() {
  ...
  // 工作区换了，桌宠记忆的缓存必须一起失效，否则新目录里的记忆读不出来。
  petMemoryCache = null;
  for (const directory of [RECORDINGS_DIR_NAME, CLIP_IMAGES_DIR_NAME]) {
...
// ============ 照片生成换肤 ============
// 照片与镜像封面同源：都放在工作区目录，跟着工作区一起迁移。
function photoThemePath() {
  return workspacePath(PHOTO_THEME_FILE);
}

function photoThemeDataUrl() {
  try {
    const image = nativeImage.createFromPath(photoThemePath());
    if (image.isEmpty()) return null;
    return image.toDataURL();
  } catch (error) {
    return null;
...
// 主进程只负责选图与落盘；色板提取在渲染层用 canvas 完成，
// 提取结果再经 normalizeThemePalette 校验后写回设置。
async function choosePhotoTheme() {
  const result = await showOwnedOpenDialog({
    title: '选择一张照片生成主题',
    properties: ['openFile'],
    filters: [{ name: '图片', extensions: ['png', 'jpg', 'jpeg', 'webp', 'heic'] }],
  ...
  try {
    ...
    const size = image.getSize();
    if (!size.width || !size.height || size.width * size.height > 60_000_000) throw new Error('image_too_large');
    fs.writeFileSync(photoThemePath(), image.toJPEG(92), { mode: 0o600 });
    return { ok: true, canceled: false, dataUrl: photoThemeDataUrl() };
  } catch (error) {
    await dialog.showMessageBox({ type: 'error', title: '无法读取照片', message: '请选择一张有效且尺寸适中的图片。' });
    return { ok: false, error: 'invalid_image' };
  }
}

function clearPhotoTheme() {
  try {
    fs.unlinkSync(photoThemePath());
  } catch (error) {
    // 文件本来就不存在不算失败。
  }
  return { ok: true };
...
function refreshTrayMenu() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    {
      // 桌宠是全屏透明覆盖窗口：留一个菜单栏开关作为兜底入口，
      // 万一渲染层出问题，用户不必依赖面板也能关掉它。
      label: '显示桌宠',
      type: 'checkbox',
      checked: settings.pet.enabled !== false,
      click: (item) => {
        const next = readAppSettings();
        next.pet = normalizePetSettings({ ...next.pet, enabled: item.checked });
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        dialog.showMessageBox({
          ...
          if (response === 0) shell.openExternal('https://github.com/moronfranklyn-lab/panel-xiaobei');
        });
      },
    },
    { type: 'separator' },
...
// ============ 换肤与桌宠 IPC ============
ipcMain.handle('settings:set-theme', (event, payload) => {
  const requested = payload && typeof payload === 'object' ? payload : {};
  const next = normalizeThemeSettings(requested.theme, requested.palette);
  if (next.theme === 'photo' && !next.palette) return { ok: false, error: 'invalid_palette' };
  const current = readAppSettings();
  if (!saveAppSettings({ ...current, theme: next.theme, themePalette: next.palette })) {
...
ipcMain.handle('theme:choose-photo', () => choosePhotoTheme());
ipcMain.handle('theme:get-photo', () => ({ ok: true, dataUrl: photoThemeDataUrl() }));
ipcMain.handle('theme:clear-photo', () => clearPhotoTheme());

ipcMain.handle('settings:set-pet', (event, payload) => {
  const requested = payload && typeof payload === 'object' ? payload : {};
  const current = readAppSettings();
  const pet = normalizePetSettings({
    enabled: requested.enabled === undefined ? current.pet.enabled : requested.enabled,
    size: requested.size === undefined ? current.pet.size : requested.size,
  });
  if (!saveAppSettings({ ...current, pet })) return { ok: false, error: 'save_failed' };
  syncPetWindow();
  const settings = publicAppSettings();
  if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('settings:changed', settings);
...
ipcMain.handle('pet:state', () => petPublicConfig());

// 渲染层量出 CSS 实际尺寸后上报，窗口大小据此计算，避免在两处硬编码同一组数值。
ipcMain.handle('pet:report-layout', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  const data = payload && typeof payload === 'object' ? payload : {};
  petLayout = {
    petSize: Math.max(32, Number(data.petSize) || petLayout.petSize),
    bubbleWidth: Math.max(120, Number(data.bubbleWidth) || petLayout.bubbleWidth),
    toastWidth: Math.max(120, Number(data.toastWidth) || petLayout.toastWidth),
  };
  const previous = petBaseSize;
  petBaseSize = computePetBaseSize();
  if (petWindow && !petWindow.isDestroyed() && petBaseSize.petSize !== previous.petSize) {
    applyPetWindowSize({ width: petBaseSize.width + petChrome.width, height: petBaseSize.height + petChrome.height });
  }
  return { ok: true, windowSize: currentPetWindowSize(), petSize: petBaseSize.petSize };
});

// 移动窗口：宠物本体在窗口内锚定，所以移动窗口就是移动宠物。
ipcMain.handle('pet:move', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  if (!petWindow || petWindow.isDestroyed()) return { ok: false, error: 'no_window' };
  const area = petWorkArea();
  const current = petWindow.getBounds();
  const data = payload && typeof payload === 'object' ? payload : {};
  const x = Number.isFinite(Number(data.x)) ? Math.round(Number(data.x)) : current.x;
  const y = Number.isFinite(Number(data.y)) ? Math.round(Number(data.y)) : current.y;
  const bounded = {
    x: Math.round(Math.min(Math.max(x, area.x), Math.max(area.x, area.x + area.width - current.width))),
    y: Math.round(Math.min(Math.max(y, area.y), Math.max(area.y, area.y + area.height - current.height))),
    width: current.width,
    height: current.height,
  };
  petWindow.setBounds(bounded);
  return { ok: true, bounds: bounded };
});

// 气泡/提示卡需要在宠物上方展开：按需向上加高窗口，宠物本身位置不变。
ipcMain.handle('pet:set-chrome', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  const data = payload && typeof payload === 'object' ? payload : {};
  petChrome = {
    width: Math.max(0, Math.min(120, Number(data.width) || 0)),
    height: Math.max(0, Math.min(360, Number(data.height) || 0)),
  };
  const bounds = applyPetWindowSize({ width: petBaseSize.width + petChrome.width, height: petBaseSize.height + petChrome.height });
  return { ok: true, bounds };
});

ipcMain.handle('pet:chat', (event, payload) => requestPetReply(payload && payload.message));

ipcMain.handle('pet:memory-get', () => readPetMemory());

ipcMain.handle('pet:memory-add', (event, payload) => {
  const content = String(payload && payload.content == null ? '' : payload.content).trim().slice(0, 2000);
  if (!content) return { ok: false, error: 'empty_content' };
  const memory = readPetMemory();
  const next = writePetMemory({
    ...memory,
    longTerm: [...memory.longTerm, { id: `mem-${Date.now()}`, content, ts: Date.now(), source: 'manual' }],
  });
  if (!next) return { ok: false, error: 'save_failed' };
  broadcastPetMemory();
  return { ok: true, memory: next };
});

ipcMain.handle('pet:memory-delete', (event, payload) => {
  const id = String(payload && payload.id || '');
  const kind = payload && payload.kind === 'longTerm' ? 'longTerm' : 'conversations';
  if (!id) return { ok: false, error: 'invalid_id' };
  const memory = readPetMemory();
  const next = writePetMemory({ ...memory, [kind]: memory[kind].filter((entry) => entry.id !== id) });
  if (!next) return { ok: false, error: 'save_failed' };
  broadcastPetMemory();
  return { ok: true, memory: next };
});

ipcMain.handle('pet:memory-clear', (event, payload) => {
  const kind = payload && payload.kind;
  const memory = readPetMemory();
  const next = kind === 'longTerm' || kind === 'conversations'
    ? writePetMemory({ ...memory, [kind]: [] })
    : writePetMemory({ conversations: [], longTerm: [] });
  if (!next) return { ok: false, error: 'save_failed' };
  broadcastPetMemory();
  return { ok: true, memory: next };
});

// 命中穿透的开关只允许桌宠窗口自己拨动，别的渲染进程无权改变整屏覆盖层的行为。
ipcMain.handle('pet:set-interactive', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  const interactive = payload && payload.interactive === true;
  petWindow.setIgnoreMouseEvents(!interactive, { forward: true });
  return { ok: true };
});

ipcMain.handle('pet:set-focusable', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  const focusable = payload && payload.focusable === true;
  petWindow.setFocusable(focusable);
  if (focusable) petWindow.focus();
  return { ok: true };
});

ipcMain.handle('pet:open-panel', () => {
  openRendererPanel('app:open-pet');
  return { ok: true };
});

ipcMain.handle('pet:save-motion', (event, payload) => {
  if (!isPetSender(event)) return { ok: false, error: 'forbidden' };
  const motion = normalizePetMotion(payload);
  const current = readAppSettings();
  saveAppSettings({ ...current, petMotion: motion });
  return { ok: true, motion };
});
ipcMain.handle('workspace:get', () => ({ path: workspaceRoot(), portable: workspaceRoot() !== app.getPath('userData') }));ipcMain.handle('workspace:load-data', () => {
...
async function promptForMissingPermissions() {
  ...
  // 屏幕录制不在这里提示：macOS 只有在应用真正发起过一次捕获之后才会把它登记进
  // 「系统设置 → 屏幕录制」，只查询状态不会登记。启动时把用户指过去，那边根本没有
  // 条目可授权。改为用户第一次真正使用「当前窗口」时按需触发（见 requestScreenRecordingAccess）。
  if (systemPreferences.isTrustedAccessibilityClient(false)) return;

  const { response, checkboxChecked } = await dialog.showMessageBox({
    type: 'info',
    message: 'TO-DO Panel 需要「辅助功能」权限',
    detail: [
      '缺少该权限时，「当前窗口」无法枚举和聚焦窗口，本地音乐的播放控制也不会生效。',
      '',
  ...
  // 传 true 触发一次系统的辅助功能提示：这一步会把应用登记进系统设置的列表里，
  // 否则用户打开设置面板可能找不到这一项、只能手动拖进去。
  systemPreferences.isTrustedAccessibilityClient(true);
  shell.openExternal(PRIVACY_SETTINGS_PANES.accessibility);
}

// 屏幕录制权限只能靠「真正发起一次捕获」来申请：
// macOS 在应用发起捕获前不会把它登记进系统设置的屏幕录制列表，所以直接打开设置页
// 是死胡同。这里在用户确实要用「当前窗口」时才调一次 getSources 触发系统授权框，
// 完成后系统列表里就有条目了（授权生效通常需要重启应用）。
async function requestScreenRecordingAccess() {
  ...
  if (policy.hasAccess) return true;
  try {
...
ipcMain.handle('windows:list', async () => {
  ...
  // 只有真正用到「当前窗口」时才申请屏幕录制：macOS 必须由一次真实捕获来登记，
  // 这里顺手触发系统授权框（每次启动最多一次），用户授权后重启即生效。
  if (result && result.error === 'screen_recording_permission_required' && !screenRecordingPromptRequested) {
...
// 渲染层权限卡片上的「请求授权」按钮：显式触发一次系统授权框。
ipcMain.handle('privacy:request-screen-recording', async () => {
  const granted = await requestScreenRecordingAccess();
  return { ok: true, granted: granted === true };
});
...
// 进程匹配用「bundle 内的可执行文件绝对路径」锚定，避免误判同名前缀进程。
function musicAppRunning(appInfo) {
  if (process.platform !== 'darwin' || !appInfo) return Promise.resolve(false);
  const executable = `${appInfo.path}/Contents/MacOS/${appInfo.executable}`;
  return new Promise((resolve) => {
    execFile('/usr/bin/pgrep', ['-f', `^${executable}$`], { timeout: 1500 }, (error) => resolve(!error));
  });
}

function launchMusicApp(appInfo) {
  return new Promise((resolve) => {
...
const MUSIC_SHORTCUT_JXA = `
function run(argv) {
  const keyCode = Number(argv[0]);
  const usesCommand = String(argv[1] || '') === '1';
  const dismissOverlays = String(argv[2] || '') === '1';
  const bundleId = String(argv[3] || '');
  const processes = Application('System Events').applicationProcesses.whose({ bundleIdentifier: bundleId })();
...
async function sendMusicShortcut(action, appInfo) {
  ...
  const shortcut = musicShortcutSpec(action);
  if (!shortcut) return { ok: false, error: 'invalid_action' };
  try {
    const result = await runJxa(MUSIC_SHORTCUT_JXA, [
      shortcut.keyCode,
      shortcut.command ? '1' : '0',
      shortcut.dismissOverlays ? '1' : '0',
      appInfo.bundleId,
    ]);
    return result === 'ok' ? { ok: true } : { ok: false, error: 'music_control_failed' };
  } catch (error) {
    console.warn('[music] failed to send music shortcut', error && error.message || error);
    return { ok: false, error: 'music_control_failed' };
  }
}

ipcMain.handle('music:status', async () => {
  const appInfo = resolveMusicApp();
  const installed = Boolean(appInfo);
  const running = installed ? await musicAppRunning(appInfo) : false;
  if (!running) musicPlaying = false;
  return {
    installed,
    running,
    label: appInfo ? appInfo.label : '',
    sessionActive: running,
    playing: running && musicPlaying,
    title: '',
    artist: '',
    icon: installed ? await readSystemAppIconNow(appInfo.path) : null,
  };
});
...
ipcMain.handle('music:control', async (event, action) => {
  if (process.platform !== 'darwin') return { ok: false, error: 'unsupported' };
  const appInfo = resolveMusicApp();
  if (!appInfo) return { ok: false, error: 'not_installed' };
  const result = await controlMusicApp(action, {
    isRunning: () => musicAppRunning(appInfo),
    launch: () => launchMusicApp(appInfo),
    sendShortcut: (target) => sendMusicShortcut(target, appInfo),
  }, musicPlaying);
  if (result && result.ok) musicPlaying = result.playing;
...
function watchDisplayChanges() {
  ...
  const reposition = () => {
    ...
    timer = setTimeout(() => {
      ...
      // 桌宠窗口锚定主面板所在屏，换屏后必须整体搬到新屏，否则宠物会留在旧屏上。
      // 位置由主进程改动，必须重新下发配置，否则渲染层仍按旧坐标推送。
      if (petWindow && !petWindow.isDestroyed()) {
        petWindow.setBounds(getPetBounds());
        sendPetConfig();
      }
    }, 100);
  };
