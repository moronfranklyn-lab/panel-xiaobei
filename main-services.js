...
const DEFAULT_PANEL_TABS = new Set(['home', 'todo', 'notes', 'links', 'recordings', 'credentials', 'clip', 'pet', 'settings']);
...
// ============ 照片生成换肤 ============
// 语义调色板：只覆盖颜色类 token，圆角/间距/字体/动效一律不动。
// 键名与顺序是稳定契约，渲染层 theme.js 与主进程校验共用同一份定义。
...
const THEME_PREFERENCES = new Set(['classic', 'photo']);
const COLOR_VALUE_PATTERN = /^(#[0-9a-fA-F]{3}|#[0-9a-fA-F]{6}|rgba?\([\d.,\s%]+\))$/;

function normalizeThemePreference(value) {
  return THEME_PREFERENCES.has(value) ? value : 'classic';
}

// 调色板来自渲染层，属于不可信输入：键必须齐全、值必须是合法颜色字面量，
// 任何一个不合法就整体拒绝，绝不半信半疑地注入 CSS。
function normalizeThemePalette(value) {
  ...
  for (const key of THEME_PALETTE_KEYS) {
    ...
    if (!COLOR_VALUE_PATTERN.test(trimmed)) return null;
    palette[key] = trimmed;
  }
  return palette;
}

// 主题是 photo 但调色板缺失/损坏时回落到 classic，避免面板停在半套颜色上。
function normalizeThemeSettings(theme, palette) {
  const normalizedTheme = normalizeThemePreference(theme);
  const normalizedPalette = normalizeThemePalette(palette);
  if (normalizedTheme === 'photo' && !normalizedPalette) return { theme: 'classic', palette: null };
  return { theme: normalizedTheme, palette: normalizedTheme === 'photo' ? normalizedPalette : null };
}

// ============ 桌宠 ============
const PET_SIZES = new Set(['small', 'medium', 'large']);
const PET_MEMORY_LIMITS = { conversations: 200, longTerm: 50 };
const PET_CHAT_CONTEXT_MESSAGES = 12;

function normalizePetSettings(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  return {
    enabled: source.enabled !== false,
    size: PET_SIZES.has(source.size) ? source.size : 'medium',
  };
}

// y 的语义由渲染层定义：0 = 屏幕底部安全线，1 = 竖直活动带顶端。
// 默认 0.12 与 renderer/pet.js 的内置默认保持一致，避免首次启动时桌宠飘高一截。
function normalizePetMotion(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const clampUnit = (input, fallback) => {
    const number = Number(input);
    if (!Number.isFinite(number)) return fallback;
    return Math.min(1, Math.max(0, number));
  };
  return {
    x: clampUnit(source.x, 0.5),
    y: clampUnit(source.y, 0.12),
    facing: source.facing === -1 ? -1 : 1,
  };
}

function normalizePetMemoryEntry(value, role) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const content = String(source.content == null ? '' : source.content).trim().slice(0, 4000);
  if (!content) return null;
  const id = String(source.id || '').trim().slice(0, 80);
  const ts = Math.max(0, Number(source.ts) || 0);
  if (role) return { id, role, content, ts };
  return { id, content, ts, source: source.source === 'auto' ? 'auto' : 'manual' };
}

// 对话与长期记忆都只保留最新的 N 条（最新在尾），超限淘汰最老。
function truncatePetMemory(memory) {
  ...
  const conversations = (Array.isArray(source.conversations) ? source.conversations : [])
    .map((entry) => normalizePetMemoryEntry(entry, entry && entry.role === 'assistant' ? 'assistant' : 'user'))
    .filter(Boolean)
    .slice(-PET_MEMORY_LIMITS.conversations);
  const longTerm = (Array.isArray(source.longTerm) ? source.longTerm : [])
    .map((entry) => normalizePetMemoryEntry(entry))
    .filter(Boolean)
    .slice(-PET_MEMORY_LIMITS.longTerm);
  return { conversations, longTerm };
}

function appendPetConversation(memory, role, content, options = {}) {
  const normalizedRole = role === 'assistant' ? 'assistant' : 'user';
  const entry = normalizePetMemoryEntry(
    { id: options.id, content, ts: options.ts },
    normalizedRole
  );
  if (!entry) return truncatePetMemory(memory);
  const current = truncatePetMemory(memory);
  return truncatePetMemory({
    ...current,
    conversations: [...current.conversations, entry],
  });
}

// 组装发给模型的 messages：人设 + 长期记忆 + 最近对话窗口 + 本轮提问。
function buildPetChatMessages(memory, userMessage, options = {}) {
  const current = truncatePetMemory(memory);
  const question = String(userMessage == null ? '' : userMessage).trim().slice(0, 2000);
  const messages = [];
  const persona = String(options.persona || '').trim();
  const memories = current.longTerm.map((entry) => entry.content).filter(Boolean);
  const systemLines = [
    persona || '你是 TO-DO Panel 工作台里的桌宠，用简短、友好、口语化的中文回答，通常不超过三句话。',
  ];
  if (memories.length) {
    systemLines.push(`关于主人的长期记忆：\n${memories.map((line) => `- ${line}`).join('\n')}`);
  }
  messages.push({ role: 'system', content: systemLines.join('\n\n') });
  for (const entry of current.conversations.slice(-PET_CHAT_CONTEXT_MESSAGES)) {
...
async function controlMusicApp(action, dependencies = {}, currentPlaying = false) {
  ...
  if (!shortcutResult || shortcutResult.ok !== true) {
    return {
      ok: false,
      error: shortcutResult && shortcutResult.error || 'music_control_failed',
      running: true,
